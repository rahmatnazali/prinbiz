...
public class WbxmlParser implements XmlPullParser {
    ...
    public void require(int type, String namespace, String name) throws XmlPullParserException, IOException {
        if (type != this.type || ((namespace != null && !namespace.equals(getNamespace())) || (name != null && !name.equals(getName())))) {
            exception("expected: " + (type == WAP_EXTENSION ? "WAP Ext." : TYPES[type] + " {" + namespace + "}" + name));
        }
    }
            ...
            if (enc == null) {
                switch (charset) {
                    ...
                    case 106:
                        ...
                        break;
                    default:
                        throw new UnsupportedEncodingException(XmlPullParser.NO_NAMESPACE + charset);
                }
            }
            this.encoding = enc;
    ...
    private final void nextImpl() throws IOException, XmlPullParserException {
        ...
        switch (id) {
            ...
            case FTPClient.PASSIVE_LOCAL_DATA_CONNECTION_MODE /*2*/:
                this.type = 6;
                char c = (char) readInt();
                this.text = XmlPullParser.NO_NAMESPACE + c;
                this.name = "#" + c;
            case FTPClient.PASSIVE_REMOTE_DATA_CONNECTION_MODE /*3*/:
                this.type = 4;
                this.text = readStrI();
    ...
    public void readAttr() throws IOException, XmlPullParserException {
        ...
        while (id != 1) {
            ...
            id = readByte();
            while (true) {
                if (id > TransportMediator.FLAG_KEY_MEDIA_NEXT || id == 0 || id == 2 || id == 3 || id == Wbxml.STR_T || ((id >= WAP_EXTENSION && id <= 66) || (id >= TransportMediator.FLAG_KEY_MEDIA_NEXT && id <= TransportMediator.KEYCODE_MEDIA_RECORD))) {
                    switch (id) {
                    ...
                    i = i2 + 1;
                    this.attributes[i2] = value.toString();
                    this.attributeCount++;
                }
            }
        }
    ...
    String resolveId(String[] tab, int id) throws IOException {
        int idx = (id & TransportMediator.KEYCODE_MEDIA_PAUSE) - 5;
        if (idx == -1) {
            this.wapCode = -1;
            return readStrT();
    ...
    void parseElement(int id) throws IOException, XmlPullParserException {
        ...
        if ((id & TransportMediator.FLAG_KEY_MEDIA_NEXT) != 0) {
            readAttr();
        }
        if ((id & WAP_EXTENSION) == 0) {
            z = true;
        } else {
            z = false;
        }
    ...
    int readInt() throws IOException {
        ...
        do {
            i = readByte();
            result = (result << 7) | (i & TransportMediator.KEYCODE_MEDIA_PAUSE);
        } while ((i & TransportMediator.FLAG_KEY_MEDIA_NEXT) != 0);
        return result;
    }
    ...
    String readStrT() throws IOException {
        ...
        int end = pos;
        while (end < this.stringTable.length && this.stringTable[end] != null) {
            end++;
        }
