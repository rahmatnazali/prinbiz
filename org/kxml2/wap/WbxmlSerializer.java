...
public class WbxmlSerializer implements XmlSerializer {
    ...
    public void checkPending(boolean degenerated) throws IOException {
        if (this.pending != null) {
            ...
            if (idx == null) {
                ...
                byteArrayOutputStream = this.buf;
                i = len == 0 ? degenerated ? idx[1] : idx[1] | 64 : degenerated ? idx[1] | TransportMediator.FLAG_KEY_MEDIA_NEXT : idx[1] | Wbxml.EXT_0;
                byteArrayOutputStream.write(i);
            }
            int i2 = 0;
            while (i2 < len) {
    ...
    public void startDocument(String encoding, Boolean standalone) throws IOException {
        ...
        if (this.encoding.toUpperCase().equals("UTF-8")) {
            this.out.write(106);
        } else if (this.encoding.toUpperCase().equals(FTP.DEFAULT_CONTROL_ENCODING)) {
            this.out.write(4);
        } else {
    ...
    public XmlSerializer startTag(String namespace, String name) throws IOException {
        if (namespace == null || XmlPullParser.NO_NAMESPACE.equals(namespace)) {
            checkPending(false);
            this.pending = name;
            this.depth++;
            return this;
    ...
    static void writeInt(OutputStream out, int i) throws IOException {
        ...
        while (i2 > 1) {
            i2--;
            out.write(buf[i2] | TransportMediator.FLAG_KEY_MEDIA_NEXT);
        }
        out.write(buf[0]);
    }
