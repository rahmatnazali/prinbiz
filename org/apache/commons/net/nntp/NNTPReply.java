...
public final class NNTPReply {
    ...
    public static boolean isInformational(int reply) {
        return reply >= HELP_TEXT_FOLLOWS && reply < SERVER_READY_POSTING_ALLOWED;
    }

    public static boolean isPositiveCompletion(int reply) {
        return reply >= SERVER_READY_POSTING_ALLOWED && reply < RequestState.REQUEST_CHECK_PRINT_COMPLETE;
    }

    public static boolean isPositiveIntermediate(int reply) {
        return reply >= RequestState.REQUEST_CHECK_PRINT_COMPLETE && reply < SERVICE_DISCONTINUED;
    }

    public static boolean isNegativeTransient(int reply) {
        return reply >= SERVICE_DISCONTINUED && reply < COMMAND_NOT_RECOGNIZED;
    }

    public static boolean isNegativePermanent(int reply) {
        return reply >= COMMAND_NOT_RECOGNIZED && reply < 600;
    }
}
