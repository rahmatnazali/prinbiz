...
public final class ColorUtils {
    ...
    private static int compositeAlpha(int foregroundAlpha, int backgroundAlpha) {
        return 255 - (((255 - backgroundAlpha) * (255 - foregroundAlpha)) / TelnetOption.MAX_OPTION_VALUE);
    }
    ...
    private static int compositeComponent(int fgC, int fgA, int bgC, int bgA, int a) {
        ...
        return (((fgC * TelnetOption.MAX_OPTION_VALUE) * fgA) + ((bgC * bgA) * (255 - fgA))) / (a * TelnetOption.MAX_OPTION_VALUE);
    }
    ...
    public static double calculateContrast(@ColorInt int foreground, @ColorInt int background) {
        if (Color.alpha(background) != TelnetOption.MAX_OPTION_VALUE) {
            throw new IllegalArgumentException("background can not be translucent: #" + Integer.toHexString(background));
        }
        if (Color.alpha(foreground) < TelnetOption.MAX_OPTION_VALUE) {
    ...
    public static int calculateMinimumAlpha(@ColorInt int foreground, @ColorInt int background, float minContrastRatio) {
        if (Color.alpha(background) != TelnetOption.MAX_OPTION_VALUE) {
            throw new IllegalArgumentException("background can not be translucent: #" + Integer.toHexString(background));
        } else if (calculateContrast(setAlphaComponent(foreground, TelnetOption.MAX_OPTION_VALUE), background) < ((double) minContrastRatio)) {
            return -1;
        } else {
            int minAlpha = 0;
            int maxAlpha = TelnetOption.MAX_OPTION_VALUE;
    ...
    public static void RGBToHSL(@IntRange(from = 0, to = 255) int r, @IntRange(from = 0, to = 255) int g, @IntRange(from = 0, to = 255) int b, @NonNull float[] outHsl) {
        ...
        outHsl[MIN_ALPHA_SEARCH_PRECISION] = constrain(s, 0.0f, 1.0f);
    ...
    public static int HSLToColor(@NonNull float[] hsl) {
        ...
        return Color.rgb(constrain(r, 0, (int) TelnetOption.MAX_OPTION_VALUE), constrain(g, 0, (int) TelnetOption.MAX_OPTION_VALUE), constrain(b, 0, (int) TelnetOption.MAX_OPTION_VALUE));
    }

    @ColorInt
    public static int setAlphaComponent(@ColorInt int color, @IntRange(from = 0, to = 255) int alpha) {
        if (alpha >= 0 && alpha <= TelnetOption.MAX_OPTION_VALUE) {
    ...
    public static void RGBToXYZ(@IntRange(from = 0, to = 255) int r, @IntRange(from = 0, to = 255) int g, @IntRange(from = 0, to = 255) int b, @NonNull double[] outXyz) {
        ...
        outXyz[MIN_ALPHA_SEARCH_PRECISION] = XYZ_WHITE_REFERENCE_Y * (((0.2126d * sr) + (0.7152d * sg)) + (0.0722d * sb));
    ...
    public static void XYZToLAB(@FloatRange(from = 0.0d, to = 95.047d) double x, @FloatRange(from = 0.0d, to = 100.0d) double y, @FloatRange(from = 0.0d, to = 108.883d) double z, @NonNull double[] outLab) {
        ...
        outLab[MIN_ALPHA_SEARCH_PRECISION] = 500.0d * (x - y);
        outLab[2] = 200.0d * (y - z);
    }
    ...
    public static int XYZToColor(@FloatRange(from = 0.0d, to = 95.047d) double x, @FloatRange(from = 0.0d, to = 100.0d) double y, @FloatRange(from = 0.0d, to = 108.883d) double z) {
        ...
        return Color.rgb(constrain((int) Math.round(255.0d * (r > 0.0031308d ? (1.055d * Math.pow(r, 0.4166666666666667d)) - 0.055d : r * 12.92d)), 0, (int) TelnetOption.MAX_OPTION_VALUE), constrain((int) Math.round(255.0d * (g > 0.0031308d ? (1.055d * Math.pow(g, 0.4166666666666667d)) - 0.055d : g * 12.92d)), 0, (int) TelnetOption.MAX_OPTION_VALUE), constrain((int) Math.round(255.0d * (b > 0.0031308d ? (1.055d * Math.pow(b, 0.4166666666666667d)) - 0.055d : b * 12.92d)), 0, (int) TelnetOption.MAX_OPTION_VALUE));
    }
    ...
    public static double distanceEuclidean(@NonNull double[] labX, @NonNull double[] labY) {
        return Math.sqrt((Math.pow(labX[0] - labY[0], 2.0d) + Math.pow(labX[MIN_ALPHA_SEARCH_PRECISION] - labY[MIN_ALPHA_SEARCH_PRECISION], 2.0d)) + Math.pow(labX[2] - labY[2], 2.0d));
    }
    ...
    public static void blendHSL(@NonNull float[] hsl1, @NonNull float[] hsl2, @FloatRange(from = 0.0d, to = 1.0d) float ratio, @NonNull float[] outResult) {
        ...
        outResult[MIN_ALPHA_SEARCH_PRECISION] = (hsl1[MIN_ALPHA_SEARCH_PRECISION] * inverseRatio) + (hsl2[MIN_ALPHA_SEARCH_PRECISION] * ratio);
    ...
    public static void blendLAB(@NonNull double[] lab1, @NonNull double[] lab2, @FloatRange(from = 0.0d, to = 1.0d) double ratio, @NonNull double[] outResult) {
        ...
        outResult[MIN_ALPHA_SEARCH_PRECISION] = (lab1[MIN_ALPHA_SEARCH_PRECISION] * inverseRatio) + (lab2[MIN_ALPHA_SEARCH_PRECISION] * ratio);
