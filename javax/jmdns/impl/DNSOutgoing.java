...
public final class DNSOutgoing extends DNSMessage {
    ...
    public static class MessageOutputStream extends ByteArrayOutputStream {
        ...
        void writeByte(int value) {
            write(value & TelnetOption.MAX_OPTION_VALUE);
        }
        ...
        void writeUTF(String str, int off, int len) {
            ...
            for (i = 0; i < len; i++) {
                ...
                if (ch >= 1 && ch <= TransportMediator.KEYCODE_MEDIA_PAUSE) {
                    utflen++;
                } else if (ch > 2047) {
                    utflen += 3;
                } else {
            ...
            for (i = 0; i < len; i++) {
                ...
                if (ch >= 1 && ch <= TransportMediator.KEYCODE_MEDIA_PAUSE) {
                    ...
                    writeByte(((ch >> DNSOutgoing.HEADER_SIZE) & 15) | 224);
                    writeByte(((ch >> 6) & 63) | TransportMediator.FLAG_KEY_MEDIA_NEXT);
                    writeByte(((ch >> 0) & 63) | TransportMediator.FLAG_KEY_MEDIA_NEXT);
                } else {
                    writeByte(((ch >> 6) & 31) | Wbxml.EXT_0);
                    writeByte(((ch >> 0) & 63) | TransportMediator.FLAG_KEY_MEDIA_NEXT);
                }
            }
        }
            ...
            while (true) {
                ...
                if (useCompression && DNSOutgoing.USE_DOMAIN_NAME_COMPRESSION) {
                    ...
                    if (offset != null) {
                        ...
                        writeByte((val >> 8) | Wbxml.EXT_0);
                        writeByte(val & TelnetOption.MAX_OPTION_VALUE);
                        return;
                    }
        ...
        void writeRecord(DNSRecord rec, long now) {
            ...
            if (rec.isUnique() && this._out.isMulticast()) {
                i = DNSRecordClass.CLASS_UNIQUE;
            } else {
                i = 0;
            }
            writeShort(i | indexValue);
            writeInt(now == 0 ? rec.getTTL() : rec.getRemainingTTL(now));
            MessageOutputStream record = new MessageOutputStream(RCommandClient.MIN_CLIENT_PORT, this._out, (this._offset + size()) + 2);
            rec.write(record);
    ...
    static {
        USE_DOMAIN_NAME_COMPRESSION = true;
    }
    ...
    public void addQuestion(DNSQuestion rec) throws IOException {
        MessageOutputStream record = new MessageOutputStream(RCommandClient.MIN_CLIENT_PORT, this);
    ...
    public boolean isQuery() {
        return (getFlags() & DNSRecordClass.CLASS_UNIQUE) == 0;
    }
